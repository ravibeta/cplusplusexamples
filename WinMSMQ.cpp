...
WinMSMQMonitor::run(void)
{
    ...
    fprintf( "%s(): MSMQ Monitoring is shutting down", __FUNCTION__);
...
WinMSMQMonitor::MonitorWorker(void)
{
    ...
    ULONG BlockSize = 4096;
		...
		HRESULT hr = queue->InitQueue(wczQueueName, wcslen(wczQueueName), wszComputerName, nLen, MQ_RECEIVE_ACCESS, MQ_DENY_NONE);
        if (FAILED(hr))
        {
               goto exit;
        }
...
exit:
    ...
    TraceAndLogInfo("%s: Monitor Worker Exiting...", __FUNCTION__);
